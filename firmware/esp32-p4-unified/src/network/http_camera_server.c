...
esp_err_t http_camera_server_start(void)
{
    ...
    /* /thermal/frame on a second httpd instance (port 81) so it doesn't
     * share a server task with the long-lived MJPEG handler. */
    httpd_config_t thermal_cfg = HTTPD_DEFAULT_CONFIG();
    thermal_cfg.server_port = 81;
    thermal_cfg.ctrl_port   = 32769;
    thermal_cfg.stack_size  = 8192;
    httpd_handle_t thermal_server = NULL;
    ESP_RETURN_ON_ERROR(httpd_start(&thermal_server, &thermal_cfg), TAG, "Failed to start thermal HTTP server");
    ESP_RETURN_ON_ERROR(httpd_register_uri_handler(thermal_server, &thermal_frame_uri), TAG, "Failed to register /thermal/frame");

    ESP_LOGD(TAG, "HTTP camera server started on port %d (thermal on %d)",
             config.server_port, thermal_cfg.server_port);
    return ESP_OK;
}
