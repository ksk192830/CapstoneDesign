...
#ifdef MOTOR_RUN_WHEEL_SELF_TEST
    esp_log_level_set("motor_control", ESP_LOG_INFO);
    printf("[MOTOR] wheel self-test: LF LR RF RR, forward/reverse\n");
    motor_control_run_wheel_self_test(MOTOR_SELF_TEST_SPEED,
                                      MOTOR_SELF_TEST_RUN_MS,
                                      MOTOR_SELF_TEST_PAUSE_MS);
    printf("[MOTOR] wheel self-test complete\n");
#endif
